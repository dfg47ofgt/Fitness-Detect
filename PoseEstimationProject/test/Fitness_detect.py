...
def Fitness_detect(User,camera,ID):
    
    fourcc = cv2.VideoWriter_fourcc(*'XVID')
    path = str(ID)
    if not os.path.isdir(path):
        os.makedirs(path)
    Filename = str(ID)+'/'+time.strftime('%Y%m%d_%H%M%S', time.localtime())+'.avi'
    print(Filename)
    out = cv2.VideoWriter(Filename,fourcc,30.0,(1740,480))
    ...
    while True: 
        ...
        #     print("status_of_use",status_of_use)
